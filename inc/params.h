...
const int Nx = 40*29 + 1;  	// Number of lattice sites in x-direction
const int Ny = 100 + 1;		// Number of lattice sites in y-direction

// Domain setup (physical)
const double height_p = 0.5;			// Domain height (m)
const double rho_p = 1.225;			// Fluid density (kg/m^3)
const double nu_p = 1.7894e-5;				// Fluid kinematic viscosity (pa s)
...
const double subTol = 1e-6;				// Tolerance for subiterations
...
const int nSteps = static_cast<int>(round(10000.0 / tStep));	// Number of timesteps
const int tinfo = 1;							// Frequency to write out info and logs
const int tVTK = 1;								// Frequency to write out VTK
...
const double ref_L = 0.5;			// Reference length (m)
const double ref_U = 0.6;			// Reference velocity (m/s)
